...
两条都不碰 GPU、Qdrant server 和网络,[CI](.github/workflows/ci.yml) 每次 push 都跑。套件规模、产品/引擎分解与测量出处见 [docs/TESTING.md](docs/TESTING.md) §1 —— 那是这个数字的唯一权威。GPU 端到端的 0-leak 回归(`eval/acl_regression.py`,跑在 WSL + 4090)、并发压测、备份演练、安全评审记录,也都在同一份文档里。
