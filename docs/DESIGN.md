...
## 2b. v0.3 团队版演化(2026-07-04)

**演化目标**:个人生产系统 → ~5-20 人团队内部系统。范围 = 此前差距分析的四个硬缺口:
多身份产品化 / 可观测性 / 并发实测 / 备份恢复 runbook。**明确不做**(v0.3):SSO/OIDC、
速率限制、HTTPS 终结(内网信任边界 + key)、连接器生态、水平扩展(嵌入式 Qdrant 天花板,留给 v2)。

**演化总原则(D12):引擎零改动。** 团队化的全部改动落在 Pharos 产品层——身份是"谁在问"
(产品关注点),ACL 语义是"能看什么"(引擎关注点,多租户机制早已完备并经 acl_regression
五用户矩阵验证)。若演化中发现必须动引擎,视为分层设计的缺陷,单独立项评审。

### D10:多身份模型 —— API key → 身份,三种模式,fail-closed

- **keys 模式**(团队):`PHAROS_KEYS_FILE` 指向 JSON(`{"keys":[{"key","name","tenant",
...
- **legacy 模式**(个人,向后兼容):不配 keys 文件、只设 PHAROS_API_KEY → 维持 v0.2 行为
  (单密钥 + 启动时绑定身份)。两者都不设 → **open-localhost 模式**(仅回环)。
  ⚠ legacy/open 都是**单身份**:所有客户端共享启动绑定的那一个身份,**不是多租户**——多用户务必用 keys 模式。
...
- **会话隔离升级**:returned_keys 登记键 = `身份名|会话id`——不同用户即使伪造相同
  X-Pharos-Session 也互不可见(v0.2 单身份下会话碰撞无害的前提在多用户下不再成立)。
- **轮换** = 编辑 keys 文件 + `systemctl restart pharos`(秒级);不做热加载(简单>优雅,
  团队规模下重启无感)。密钥生成器:`pharos keys new <name> --tenant --principals`。
- **否决的备选**:SSO/OIDC(v0.3 规模不值得引入 IdP 依赖);数据库存密钥(文件+重启足够);
  热加载(增加状态一致性面,收益小)。
...
- **指标**:进程内存(计数器 + 每端点延迟环形队列)→ `/v1/stats`(keys 模式下 admin key
  才可读——聚合查询模式也是信息);重启归零(v0.3 接受,持久化指标留给需要时)。
