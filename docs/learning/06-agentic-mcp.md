...
**Q9:业界都在押注 agentic 检索(deep research 满天飞),你的净负结论是不是反潮流?**
要点:把"方向"和"workload"拆开答。方向是真的——TREC RAG track 2026 版已转 agent-first,
BrowseComp 系(含中文 ZH、固定语料 Plus)这批 agentic 检索基准 2025–26 密集出现(2026-07 口径);
但它们测的题型是"开放网络 / 大语料里多步寻址难找的信息",而本项目 workload 是单库、单跳为主、
表格密集——paired 实测在**这个分布**上编排净负(Δ 约 −0.1,方向稳、幅度带 eval#0 组装偏置
待重评,见 §3.3)。所以反的不是潮流,是"不看 workload 就默认上 agent"。
加分细节:BrowseComp-Plus 特意做固定语料版,把"检索器强"从"agent 强"里解耦——社区自己也承认
这两件事必须分开度量。我们的实验意图同向:共用同一检索后端、只变编排——而 eval#0 恰恰暴露了
生成端组装在 eval 侧的漂移(见 §3.3),反过来证明这种解耦有多难、也多必要;这段自我纠错本身
就是答案的一部分。
