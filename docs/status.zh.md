...
> 不是开箱即用的产品，也不是 semver 意义上的"已发布"。本页是关于"能用什么、不能用
> 什么、真实风险在哪"的诚实交代。
...
- 核心不变量 —— 跨进程 maintenance gate、两阶段 audit、evidence/hint 分区、navigation-first
  检索 —— 都已就位且有测试。
- 在维护者机器上以参考模型栈端到端运行（Qwen3-VL 嵌入+重排、MILCO 学习式稀疏、Qwen3.6-VL
  页面描述、MinerU 版面解析）。
...
- **复现性只做了轻度验证。** 从源码安装（`pip install -e ".[server,dev]"`）加 `import kb`
  ...
  这一点、加上更重的可选 `[mineru]` extra 和 live 模型服务，是未决的 gap。
- **评测体系仍在完善。** golden set 大部分是合成的；广泛的真实文档评估是当前最大的开放质量风险。
...
争论，跟打包打磨到什么程度无关。公开代码让这个讨论从假设变成具体。

第二，**vaporware 又便宜又没用；诚实的 alpha 才有用。** 与其捂着代码等一切完美，不如交出一个
...
我们不给日历日期 —— 内部设过的每个日期都跳票了。上面是诚实的 backlog，不是发布时间表。
