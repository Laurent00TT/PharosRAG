...
class Generator:
    ...
    def answer(self, query: str, user, top_k=None, rerank: bool = False,
               doc_ids=None, doc_type=None, kind=None, strategy=None, extra_legs=None) -> Answer:
        ...
        if not contexts:   # R3.E:零召回 -> 确定性返回"信息不足",不把作答权交给 LLM(grounding 退路不再靠模型听话)
            return Answer(text="I don't have enough information in the provided context to answer.",
                          citations=[], n_contexts=0, raw_messages=messages)
        raw = self.llm.complete(messages)
        return Answer(text=raw, citations=self._parse_citations(raw, meta),
                      n_contexts=len(contexts), raw_messages=messages)

    @staticmethod
    def _parse_citations(answer_text: str, meta: list[dict]) -> list[Citation]:
        """从答案里抽 [n] 标记,映射回 context 来源(只保留实际引用且编号有效的,去重升序)。"""
        used = sorted({int(m) for m in re.findall(r"\[cite:(\d+)\]", answer_text)})   # 只认 [cite:n],不抓正文裸 [n](review#1)
        out = []
        for n in used:
