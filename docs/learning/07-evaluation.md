...
这一层有个必然的追问:"业界不是有一堆现成基准吗?"有,而且值得按四层记清楚(2026-07 口径)——但看完你会发现它们都替代不了造考卷:

| 层 | 代表 | 能统一什么 |
|---|---|---|
| 组件层 | MTEB(2026 已换代 v2,与 v1 分数不可比)、BEIR、MS MARCO | 最接近"统一":embedding / reranker 选型看榜即可——只测组件,不依赖你的语料 |
| 端到端层 | TREC RAG track(NIST,2024 起;2026 版已转 agent-first)、CRAG、RGB、RAGBench、RAGTruth(严格说是幻觉标注语料) | 统一的是**方法论与工具链**;语料仍是官方的,测的是"你的管道在别人的语料上" |
| 框架层 | ragas(RAG 细分的事实标准)、TruLens、ARES、DeepEval | 统一的是**指标协议**,数据自备——这类框架的存在本身,就是"没有统一数据集"的证明 |
| agentic 层(2025–26 新增) | BrowseComp / BrowseComp-ZH / BrowseComp-Plus、DeepResearch Bench | 评测对象从单轮管道变成多步搜索 agent;Plus 用固定语料把"检索器强"与"agent 强"解耦 |

TREC 2026 那格值得一句展开(三个新名词免得冷读卡壳):这一版换了官方语料 ClimbMix、发布了配套工具包 RAGDoll,评分用 nugget 法——把标准答案拆成事实要点、按覆盖度计分,把主观打分换成可数事实,与本篇 citation_recall 的思路同一路数。

中文侧还有 CRUD-RAG(学术主力,按增删改查四场景组卷)、SuperCLUE-RAG(同一题"先不给文档、再给文档"问两遍,测 RAG 增益)与 OpenCompass 榜。

为什么统一不了?**RAG 的输出是"管道 × 语料"的函数**,公开基准只能带自己的语料来。MMLU 那类 LLM 基准能统一,是因为被测对象自包含;RAG 不是。业界收敛出的共识因此是两层:**组件选型看公开榜,系统验收自建 golden set**——造考卷不是"没有基准"之下的替代品,是正解本身。(顺带一条纪律呼应:MTEB v2 与 v1 分数不可比,和 §3 备选 E 的"72→88 口径断代"是同一件事——换了考卷就重画基线,榜单也不例外。)
...
**备选 A:直接用 ragas / 现成评估框架。** 否决理由有三:① ragas 默认单模型裁判,同厂循环偏差没有解;② 它测不了本项目特有的资产(citation_recall 依赖自家 `[cite:n]` 结构化引用协议、ACL 隔离依赖自家权限模型);③ 私有研报数据要外发第三方 API。保留其思想(faithfulness/correctness 双裁判指标),自建管道。补一个现状(2026-07 口径):ragas 仍是 RAG 细分评测的事实标准——指标库已扩到 30+(其中 RAG 专用类 8 个);更广域的 LLM 评测框架(如 DeepEval)按 GitHub star 已反超,但 RAG 场景里它引用最广。正因如此"为什么不用"才是必问题,答案见追问 Q1/Q9。
...
**Q9:RAG 评测有没有权威、统一的基准?你这五个指标是自创的吗?**
要点(前半问):分层答(2026-07 口径,四层表见 §1)。组件层最接近统一——embedding/reranker 看 MTEB(已换代 v2)/BEIR;端到端最权威的是 TREC RAG track(NIST,2026 版已转 agent-first),但它统一的是方法论和工具链,语料仍是官方的;框架层 ragas 是 RAG 细分的事实标准;agentic 层 2025–26 新出一批(BrowseComp 系、DeepResearch Bench)。核心论点一句话:**RAG 性能 = 管道 × 语料,公开基准答不了"在我的语料上行不行",业界共识就是公开榜选组件 + 自建 golden set 验收**。
要点(后半问):五指标不是自创——忠实度/正确性与 ragas 的 faithfulness / answer correctness 同名同义;检索召回/MRR 是它 context recall/precision 的程序化加严版(我们有 golden_chunk_id,可以精确计数,不用 LLM 估计);引用召回是它默认没有的加强项。加分点:MTEB v2 与 v1 分数不可比——榜单也讲口径断代,和我们 72→88 是同一条纪律。关键词:管道×语料、公开榜选组件、golden set 验收、程序化加严。
...
9. **"负拒绝"没有专项考题。** RGB 把"语料里没有答案时该拒答"列为 RAG 四能力之一;我们的 gold 全部由语料生成、题题有答案,这个能力只被"闭管道零召回时确定性拒答"间接覆盖——**"检索到相近但不含答案的干扰块时会不会硬答"未测**。补法明确:造一批 unanswerable 题(问语料外的实体/数字),验收标准即拒答率,成本低、可程序化判定。
