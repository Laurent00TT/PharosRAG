...
def create_app(cfg: config.PharosConfig | None = None, retriever=None, user=None,
    ...
    state.generator_factory = generator_factory or engine.build_generator
    state.gen_lock = threading.Lock()
    ...
    def retrieve(q: RetrieveReq, request: Request):
        return tc._retrieve_impl(state.retriever, state.user, q.query, q.top_k, q.rerank,
                                 q.doc_ids, q.doc_type, q.kind, q.mode, q.strategy,
                                 q.rerank_top_n, returned_keys=_session_keys(request))

    @app.get("/v1/documents")
    def list_documents():
        return tc._list_impl(state.retriever, state.user)

    @app.get("/v1/documents/{doc_id}")
    def get_document(doc_id: str, max_tokens: int = 6000):
        return tc._get_document_impl(state.retriever, state.user, doc_id, max_tokens)

    @app.get("/v1/documents/{doc_id}/outline")
    def get_outline(doc_id: str):
        return tc._outline_impl(state.retriever, state.user, doc_id)

    @app.post("/v1/expand")
    def expand(q: ExpandReq):
        return tc._expand_impl(state.retriever, state.user, q.chunk_id, q.target_tokens)

    @app.post("/v1/retrieve_grouped")
    def retrieve_grouped(q: GroupedReq):
        return tc._grouped_impl(state.retriever, state.user, q.query, q.doc_ids, q.top_k, q.rerank)

    # ---------- 闭管道问答(generator:检索 + grounding prompt + DeepSeek + 引用解析)----------
    def _get_generator():
        with state.gen_lock:
            if state.generator is None:
                state.generator = state.generator_factory(state.retriever, cfg)
            return state.generator

    @app.post("/v1/ask")
    def ask(q: AskReq):
        if not state.user or not state.user.tenant:
            return {"status": "no_identity", "retriable": False, "hint": tc._NO_IDENTITY_HINT}
